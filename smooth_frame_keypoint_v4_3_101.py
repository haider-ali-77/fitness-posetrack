...
def optimal_frame_foot_on_ground_back_front_all_points(all_hip_coord, all_knee_coord, all_ankle_coord, all_knee_coord_anotherleg, all_ankle_coord_anotherleg, image_name_list, all_shoulder_coord, angle_list,path):
    ...
    if min_angle != 0:
        ...
        image = cv2.line(img, (newLine[0],newLine[1]), (newLine[2],newLine[3]), color, thickness)
        image = cv2.putText(image, str((round(min_angle, 1))),
                        (int(newLine[2]+10), int(newLine[3])), font, fontScale, fontColor, lineType)
        cv2.imwrite(path_final,image)
...
def main(args):
 ...
 # processing_time = process_endtime - process_starttime
        ...
        if (i != 0 and i % (skip_frames + 1) != 0 and skip_frames != 0 and i > start_tracking):
            ...
            num_keypoints = relevant_keypoint.shape[0]
                ...
                if (right_knee[0] > right_hip[0] and right_ankle[0] < right_knee[0]) or (left_knee[0] > left_hip[0] and left_ankle[0] < left_knee[0]):
                    ...
                    if len(right_hip) > 0 and len(right_knee) > 0 and len(right_ankle) > 0 and len(left_hip) > 0 and len(left_knee) > 0 and len(left_ankle) > 0 and left_shoulder[1] < left_hip[1] and left_hip[1] < left_ankle[1] and left_hip[1] < right_ankle[1] and left_hip[1] < right_knee[1] and left_hip[1] < left_knee[1]:
                        ...
                        if len(left_hip) > 0 and len(left_ankle) > 0 and len(right_ankle) > 0:
                            ...
                            # pelvis tilt
                            left_pelvis_tilt = []
                            distance = int((left_knee[0] - left_ankle[0]))
                            x1 = left_shoulder[0] - 60
                            y1 = (left_shoulder[1] + right_hip[1])/2
                            x2 = left_hip[0] + 40
                            y2 = left_hip[1]
                            left_pelvis_tilt.append(str(image_actual_name) + "." + args.image_ext)
                            left_pelvis_tilt.append(x1)
                            left_pelvis_tilt.append(y1)
                            left_pelvis_tilt.append(x2)
                            left_pelvis_tilt.append(y2)
                            all_leftview_left_pelvis_tilt.append(left_pelvis_tilt)
                ...
                if (left_hip[0] > left_knee[0] and left_ankle[0] > left_knee[0]) or (right_hip[0] > right_knee[0] and right_ankle[0] > right_knee[0]):
                    ...
                    if len(right_hip) > 0 and len(right_knee) > 0 and len(right_ankle) > 0 and len(left_hip) > 0 and len(left_knee) > 0 and len(left_ankle) > 0 and right_shoulder[1] < right_hip[1] and right_hip[1] < right_ankle[1] and right_hip[1] < left_ankle[1] and right_hip[1] < right_knee[1] and right_hip[1] < left_knee[1]:
                        ...
                        if len(right_hip) > 0 and len(left_ankle) > 0 and len(right_ankle) > 0:
                            ...
                            y2 = right_hip[1]
                            right_pelvis_tilt.append(str(image_actual_name) + "." + args.image_ext)
    ...
    if alignment == "left_side":  # or alignment=="right_side":
        ...
        all_leftview_left_pelvis_tilt= np.array(all_leftview_left_pelvis_tilt)
        np.save('/opt/detectron2/tools/101/all_leftview_left_pelvis_tilt.npy', all_leftview_left_pelvis_tilt)
    ...
    if alignment == "right_side":
        ...
        all_rightview_right_pelvis_tilt= np.array(all_rightview_right_pelvis_tilt)
        np.save('/opt/detectron2/tools/o101/all_rightview_right_pelvis_tilt.npy', all_rightview_right_pelvis_tilt)
    ...
    if alignment == "back_side":
        ...
        if args.pelvis == 'on':
            optimal_leftfoot_on_ground_pelvis_drop, img_leftfoot_ground_pelvis_drop,angle = optimal_frame_foot_on_ground_back_front_all_points(
                all_back_hip_coor, all_back_left_knee_coor, all_back_left_ankle_coor, all_back_right_knee_coor, all_back_right_ankle_coor, write_image_back_left_hip_knee_ankle, all_back_shoulder_coor, all_back_hip_coor_left_right,back_side_left_image_path)
    ...
    if alignment == "front_side":
        ...
        if args.pelvis == 'on':
            ...
            optimal_rightfoot_on_ground_pelvis_drop, img_rightfoot_ground_pelvis_drop,angle = optimal_frame_foot_on_ground_back_front_all_points(
                all_front_hip_coor, all_front_right_knee_coor, all_front_right_ankle_coor, all_front_left_knee_coor, all_front_left_ankle_coor, write_image_front_left_hip_right_hip, all_front_shoulder_coor, all_front_hip_coor_left_right, front_right_image_path)
        ...
        # pelvis angle
            optimal_leftfoot_on_ground_pelvis_drop, img_leftfoot_ground_pelvis_drop,angle = optimal_frame_foot_on_ground_back_front_all_points(
                all_front_hip_coor, all_front_left_knee_coor, all_front_left_ankle_coor, all_front_right_knee_coor, all_front_right_ankle_coor, write_image_front_left_hip_right_hip, all_front_shoulder_coor, all_front_hip_coor_left_right,front_left_image_path)
